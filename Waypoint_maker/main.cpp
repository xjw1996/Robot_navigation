...
int main(int argc, char **argv)
{
    ...
    if (co.event_flag()==true){
			yaw_base = yaw_now;
			if (co.erase_flag(diff_yaw,diff_yaw ,way_count)==true){
				waypoint.pop_back();
				way_count--;
			}
			waypoint.push_back(tmp_point);
			co.IN_POINT(tmp_point.x,tmp_point.y,tmp_point.z,tmp_point.ox,tmp_point.oy,tmp_point.oz,tmp_point.ow,way_count);
			way_count++;
		}else if((diff_yaw * 180.0 / M_PI > deg_thresh && diff_way > deg_distance) || diff_way > point_distance)
		{
			yaw_base = yaw_now;
			waypoint.push_back(tmp_point);
			way_count++;
		}
    co.event_flag_to_false();
		// publishするマーカの設定
